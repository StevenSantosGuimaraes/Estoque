package br.com.udf.estoque.repository;

import android.content.Context;

import java.util.List;

import br.com.udf.estoque.asynctask.BaseAsyncTask;
import br.com.udf.estoque.database.EstoqueDatabase;
import br.com.udf.estoque.database.dao.ProdutoDAO;
import br.com.udf.estoque.model.Produto;
import br.com.udf.estoque.retrofit.EstoqueRetrofit;
import br.com.udf.estoque.retrofit.callback.CallbackComRetorno;
import br.com.udf.estoque.retrofit.callback.CallbackSemRetorno;
...
public class ProdutoRepository {
    ...
    private void salvaNaAPI(Produto produto, DadosCarregadosCallback<Produto> callback) {
        ...
        call.enqueue(new CallbackComRetorno<>(new CallbackComRetorno.RespostaCallback<Produto>() {
            ...
            public void quandoSucesso(Produto produtoSalvo) {
                salvarInterno(produtoSalvo, callback);
            }
            @Override
            public void quandoFalha(String erro) {
                callback.quandoFalha(erro);
    ...
    private void salvarInterno(Produto produto, DadosCarregadosCallback<Produto> callback) {
        new BaseAsyncTask<>(() -> {
